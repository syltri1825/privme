...
export default function UpdFormImage({className, setting, labelPrice, pile, info, link, update, title, infoObject, description, category}){
    ...
    const validPortSubmit = (e) => {
        e.preventDefault();
        if(addList.title != '' && addList.category != '' && addList.price != '' &&
        addList.description != '' && objectFile.file != null){
            ...
            Inertia.post(`/${link}/${update}`,portData);
            // Inertia.put(`/formupdimage/${update}`,portData);
            // Inertia.put(`/formupdimage/${update}`,portData,{
            //     forceFormData: true,
            // });
        }
