...
export default function AdminSmartPhone(props) {
    ...
    return (
        <ActionLayouts
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold">Post with Smartphone</h2>}
            add={route('addSmartphone')}
            activeSmt={route().current('smartphone')}
            // update={route('updateimage')}
        >
            <Head title="Post with Smartphone" />

            <Title intituleTitle={<h3>All your Smartphone publications</h3>} ></Title>
                ...
                props.smartphone.map(p => 
                    (
                        <Card
                                ...
                                more={`/dashboard/details/smartphone/${p.id}`}
                                testMe={"one"}
                        >
                        </Card>
                    )
